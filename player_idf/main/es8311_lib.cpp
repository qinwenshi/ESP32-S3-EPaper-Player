...
bool ES8311::begin(int32_t sda, int32_t scl, uint32_t frequency) {
    ...
    // ── Init phase (matching Espressif ESP-ADF es8311.c: es8311_codec_init) ────
    // Software reset (board-level in ESP-ADF, we do it here)
    WriteReg(0x00, 0x1F);
    vTaskDelay(20 / portTICK_PERIOD_MS);
    WriteReg(0x00, 0x00);

    WriteReg(0x44, 0x08);  // Enhance I2C noise immunity (write twice per ESP-ADF)
    WriteReg(0x44, 0x08);

    ok |= WriteReg(0x01, 0x30);  // Clocks: dividers on, but all clock gates off initially
    ok |= WriteReg(0x02, 0x00);  // CLK2: pre-div=1, pre-mult=1
    ok |= WriteReg(0x03, 0x10);  // ADC OSR = 0x10
    ok |= WriteReg(0x04, 0x10);  // DAC OSR = 0x10
    ok |= WriteReg(0x05, 0x00);  // ADC/DAC clock dividers
    ok |= WriteReg(0x0B, 0x00);  // System REG0B
    ok |= WriteReg(0x0C, 0x00);  // System REG0C
    ok |= WriteReg(0x10, 0x1F);  // VDAC reference + analog bias power
    ok |= WriteReg(0x11, 0x7F);  // Analog gain reference

    ok |= WriteReg(0x00, 0x80);  // CSM_ON: trigger chip startup sequence

    // Slave mode: read reg00, clear bit6 (master), write back
    reg = ReadReg(0x00);
    reg &= 0xBF;  // bit6 = 0 → I2S slave
    ok |= WriteReg(0x00, reg);

    ok |= WriteReg(0x01, 0x3F);  // Enable all clock gates

    // MCLK from MCLK pin: clear bit7 of reg01
    reg = ReadReg(0x01);
    reg &= 0x7F;
    ok |= WriteReg(0x01, reg);
    ...
    ok |= setSampleRate(ES8311_SAMPLE_RATE48);   // default; call setSampleRate() again after begin()
    ok |= setBitsPerSample(16);                  // 16-bit: Audio lib packs samples in bits[31:16] of 32-bit I2S slot

    ok |= WriteReg(0x13, 0x10);  // HP drive enable
    ok |= WriteReg(0x1B, 0x0A);  // ADC REG1B (per ESP-ADF)
    ok |= WriteReg(0x1C, 0x6A);  // ADC equalizer bypass, DC offset cancel

    // ── Start phase (matching ESP-ADF es8311_start: ES_MODULE_DAC) ───────────
    // Unmute SDP input (reg09 bit6 = 0 = not muted)
    reg = ReadReg(0x09);
    reg &= 0xBF;
    ok |= WriteReg(0x09, reg);

    ok |= WriteReg(0x0E, 0x02);  // Enable analog PGA + ADC modulator
    ok |= WriteReg(0x12, 0x00);  // Power-up DAC
    ok |= WriteReg(0x14, 0x1A);  // Analog output enable (LINEOUT/HP)
    ok |= WriteReg(0x0D, 0x01);  // Power up analog circuitry
    ok |= WriteReg(0x15, 0x40);  // ADC equalizer
    ok |= WriteReg(0x37, 0x08);  // Bypass DAC equalizer
    ok |= WriteReg(0x45, 0x00);  // GP REG45
    ok |= WriteReg(0x44, 0x58);  // Internal reference signal (ADCL + DACR)

    // Unmute DAC: reg31 bits[6:5] = 00 (per ESP-ADF es8311_mute(0))
    reg = ReadReg(0x31);
    reg &= 0x9F;                 // clear mute bits — 0x60 would be MUTED!
    ok |= WriteReg(0x31, reg);
...
bool ES8311::setVolume(uint8_t volume){ // 0...100
    ...
    // ES8311 reg0x32: 0x00=mute, 0xFF=max(+32dB), 0xBF≈0dB
    // Map 0-100 linearly to 0-0xFF (matches ESP-ADF es8311_set_voice_volume)
    uint8_t reg32 = (uint8_t)((uint16_t)volume * 255 / 100);
...
bool ES8311::setBitsPerSample(uint8_t bps){
    ...
    reg09 &= ~(0x1C);  // clear bits [4:2] before setting
    reg0A &= ~(0x1C);
    switch (bps) {
